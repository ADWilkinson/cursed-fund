      ...
      <div className="block  bg-theme-pan-champagne border-t border-b mb-3 border-theme-pan-navy   divide-theme-pan-navy">
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            <div className="sm:flex sm:items-center">
              <div className="sm:flex-auto">
                <h1 className="text-xl font-semibold text-theme-pan-navy">Royal Fortune Fund Snapshots</h1>
                  ...
                  <div className="overflow-hidden border border-theme-pan-navy rounded-2xl">
                    <table className="min-w-full divide-y divide-theme-pan-navy">
                      <thead className="bg-white">
                        <tr>
                          <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-theme-pan-navy sm:pl-6">
                            Time
                          </th>
                          <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-theme-pan-navy">
                            Total Votes
                          </th>
                          <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-theme-pan-navy">
                            WETH Allocation
                      ...
                      <tbody className="divide-y divide-theme-pan-navy bg-white">
                        {galleonSnapshots.length === 0 ? <>
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">-</td>
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">-</td>
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">-</td>
                        </> : (<>
                          {galleonSnapshots.map((snapshot) => (
                            <tr key={snapshot.timestamp}>
                              ...
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">{snapshot.totalVotes}</td>
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">{snapshot.percentBullish}%</td>
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">{snapshot.percentBearish}%</td>
                            </tr>
                          ))}</>)}
                      </tbody>
                    </table>
        ...
        <div className="w-full items-center justify-between p-6 border-none space-x-6">
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            <div className="sm:flex sm:items-center">
              <div className="sm:flex-auto">
                <h1 className="text-xl font-semibold text-theme-pan-navy">Control Point Snapshots</h1>
                <p className="mt-2 text-sm text-theme-pan-sky">
                  Winner of the weekly Control Point Fund & target fund allocation.
                </p>
              </div>
                        ...
                        {galleonSnapshots.length === 0 ? <>
                          ...
                          {controlSnapshots.map((snapshot) => (
                            <tr key={snapshot.timestamp}>
                              ...
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">{`${snapshot.account.slice(0, 6)}...${snapshot.account.slice(snapshot.account.length - 4, snapshot.account.length)}`}</td>
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-theme-pan-navy">100% {snapshot.isBullish ? 'WETH' : 'USDC'}</td>
                            </tr>
                          ))}</>)}
