...
const ControlExperiment = () => {
  ...
  useEffect(() => {
    ...
    const controlQuery = query(
      collection(db, CONTROL_COLLECTION),
      orderBy("timestamp", "desc"),
      limit(1)
    );
    getDocs(controlQuery).then((doc) => {
      setControl(doc.docs.pop().data());
    });
  }, [account, state, chainId]);
        ...
        <div className="w-full items-center justify-between p-6 border-none space-x-6">
          {account && chainId && chainId === ARBITRUM.chainId ? (
            <div className="text-theme-pan-navy">
              ...
              <div>
                {parseFloat(displayFromWei(balances.ethBalance, 3)) <
                ETH_MINIMUM ? (
                  <div className="border-t border-b border-theme-pan-navy mb-3">
              ...
              >
                ...
                <p className="">
                  ...
                  at a 0.001 ETH cost. The deckhand with the most captures by
                  the end of the week wins 1% of the fund.
                </p>
                    ...
                    <span className="p-4 rounded-2xl bg-theme-white mt-4 border border-theme-pan-navy">
                      {control.account !== "" ? (
                        <>
                          <p>
                            Controller:{" "}
                            <span className="font-bold animate animate-pulse ">
                              {control.account.slice(0, 6)}...
                              {control.account.slice(
                                control.account.length - 4,
                                control.account.length
                              )}
                            </span>
                          </p>
                          <p>
                            ...
                            <span className="font-bold animate animate-pulse ">
                              {control.isBullish ? "Bullish" : "Bearish"}
                            </span>
                          </p>
                          {currentWinner !== "" ? (
                            <>
                              ...
                              <p>
                                ...
                                <span className="font-bold">
                                  {currentWinner.slice(0, 6)}...
                                  {currentWinner.slice(
                                    currentWinner.length - 4,
                                    currentWinner.length
                                  )}
                                </span>
                              </p>
                              <p>
                                Leader Sentiment{" "}
                                <span className="font-bold">
                                  {winnerSentiment ? "Bullish" : "Bearish"}
                                </span>
                              </p>
                              <p>
                                Potential Reward:{" "}
            ...
            <>
              <div className=" px-2 pb-4 pt-4 sm:px-4">
                ...
                <p>Please switch network to capture the point.</p>
                <>
                  <button
                    onClick={() => changeNetwork(ARBITRUM.chainId.toString())}
        ...
        <div className="w-full items-center justify-between p-6 space-x-6">
          <div>
            ...
            <a
              href={CONTROL_ARBISCAN}
              target={"_blank"}
              className="mx-auto mt-6 overflow-hidden  text-center py-2 justify-center flex hover:opacity-70 items-center rounded-xl bg-theme-pan-navy  px-2.5 text-sm font-medium text-theme-pan-champagne"
              rel="noreferrer"
            >
